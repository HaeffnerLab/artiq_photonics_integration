...
class A6_Displacement_Wigner(_ACFExperiment):
    ...
    @kernel
    def rabi_AWG(self, beta_time, freq_729_dp):
        ...
        if self.vib_mode=="mode1":
            self.seq.sdf_mode1.run(self.drive_time_us)
        elif self.vib_mode=="mode2":
            self.seq.sdf_mode2.run(self.drive_time_us)
        elif self.vib_mode=="mode_single_ion":
            self.seq.sdf_single_ion.run(self.drive_time_us)
        else:
    ...
    def exp_seq(self, freq_729_dp, beta_time, ion_status_detect):
        ...
        # rabi 
        self.rabi_AWG(beta_time, freq_729_dp+freq_diff_dp)
