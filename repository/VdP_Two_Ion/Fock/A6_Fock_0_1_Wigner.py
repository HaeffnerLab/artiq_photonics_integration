...
class A6_Fock_0_1_Wigner(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "att_729_dp_pi",
            NumberValue(default=self.parameter_manager.get_param("Fock/att_729_dp"), min=15*dB, max=30*dB, unit="dB", precision=8),
            tooltip="729 double pass amplitude for resonance",
            group='Pi pulse excitation'
        )
        self.setattr_argument(
            "freq_729_sp_pi",
            NumberValue(default=self.parameter_manager.get_param("frequency/729_sp")-self.parameter_manager.get_param("Fock/freq_BSB_sp"), min=20*MHz, max=250*MHz, unit="MHz", precision=8),
            tooltip="729 double pass frequency for resonance",
            group='Pi pulse excitation'
        )
        self.setattr_argument(
            "att_729_sp_pi",
            NumberValue(default=13.0*dB, min=10*dB, max=30*dB, unit="dB", precision=8),
            tooltip="729 double pass amplitude for resonance",
            group='Pi pulse excitation'
        )
        self.setattr_argument(
            "PI_drive_time",
            NumberValue(default=self.parameter_manager.get_param("Fock/pi_time_BSB"), min=0.*us, max=1000*us, unit='us', precision=8),
        ...
        self.setattr_argument(
            "beta_range_us",
            NumberValue(default=60*us, min=0.001*us, max=300*us,  precision=8, unit='us'),
        ...
        self.setattr_argument(
            "drive_time_factor",
            NumberValue(default=0.8, precision=8, step=1),
            tooltip="Drive time factor",
        )
        ...
        self.setattr_argument("enable_mid_detection", BooleanValue(False))
    ...
    @kernel
    def exp_seq(self, freq_729_dp, beta_time, ion_status_detect)->bool:
        #854 repump
        ...
        # rabi 
        self.dds_729_dp.set_att(self.att_729_dp_pi)
        self.dds_729_dp.set(freq_729_dp)
        self.dds_729_sp.set(self.freq_729_sp_pi)
        self.dds_729_sp_aux.set(self.freq_729_sp_pi)
        self.dds_729_sp.sw.on()
        self.dds_729_dp.sw.on()

        delay(self.PI_drive_time*self.drive_time_factor)
    ...
    def send_exp_para(
        ...
        send_exp_para(["SDF", {
                "freq_sp_RSB":   self.seq.sdf_single_ion.freq_729_sp_aux,
                "freq_sp_BSB":   self.seq.sdf_single_ion.freq_729_sp,
                "amp_sp_RSB":  self.seq.sdf_single_ion.amp_729_sp_aux,
                "amp_sp_BSB":   self.seq.sdf_single_ion.amp_729_sp,
                "phase": phase,
    ...
    def run(self):
        ...
        while i < self.num_samples:
            ...
            if self.seq.calibrate_motion.check_interval():
                ion_status_detect=self.seq.calibrate_motion.calibrate_freq_motion(vib_mode="mode_single_ion")
                ion_status_detect_last=ion_status_detect
            ...
            self.send_exp_para(self.beta_phase[i]*360)
            sample_num=0
            ...
            while sample_num<self.samples_per_time:
                if ion_status_detect==1 or ion_status_detect==2:
                    ...
                    if not self.exp_seq(freq_729_dp, self.beta_time[i], ion_status_detect):
                        continue
                    ################################################################################ get readout
