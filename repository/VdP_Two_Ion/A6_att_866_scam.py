...
class A6_att866_scan_AWG_Cam(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument("cooling_option", EnumerationValue(["sidebandcool", "sidebandcool2mode", "opticalpumping"], default="sidebandcool2mode"))
    ...
    @kernel
    def exp_seq(self, att_866_dp, ion_status_detect):
        #854 repump
        ...
        if self.cooling_option == "sidebandcool":
            ...
            if ion_status_detect==2:
                self.seq.sideband_cool_2mode.run(att_866_here=att_866_dp,  freq_diff_dp=self.freq_diff_dp) 
            else:
                self.seq.sideband_cool_2mode.run(att_866_here=att_866_dp)
    ...
    def run(self):
        ...
        i=0
        while i < len(self.scan_att_866.sequence):

            total_thresh_count = 0
            sample_num=0
            att_866_dp =self.scan_att_866.sequence[i]
            num_try_save_ion = 0 
            ...
            while sample_num<self.samples_per_time:
                if ion_status_detect==1 or ion_status_detect==2:
                    ...
                    self.exp_seq( att_866_dp, ion_status_detect)
                    ################################################################################
            ...
            self.experiment_data.append_list_dataset("attenuation_dB", att_866_dp/dB)
