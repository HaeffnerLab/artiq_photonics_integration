...
class Rabi_Freq(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "rabi_t",
            NumberValue(default=300*us, min=0*us, max=5*ms, unit="us",precision=8),
            group="rabi"
        )
        self.setattr_argument(
            "att_729_dp",
        ...
        self.setattr_argument(
            "att_729_sp",
            NumberValue(default=self.parameter_manager.get_param("attenuation/729_radial_sp"), min=0., max=31.0, unit="dB", precision=8),
            tooltip="729 double pass attenuation",
            group="rabi"
        )
        ...
        self.setattr_argument(
            ...
            Scannable(
                default=RangeScan(
                    start=self.parameter_manager.get_param("qubit/Sm1_2_Dm5_2")-0.01*MHz,
                    stop=self.parameter_manager.get_param("qubit/Sm1_2_Dm5_2")+0.01*MHz,
                    npoints=40
                ),
                global_min=150*MHz,
                global_max=250*MHz,
        ...
        self.setattr_argument(
            "freq_diff_dp",
            NumberValue(default=self.parameter_manager.get_param("VdP2mode/freq_diff_dp"), min=-56.0*MHz, max=250.0*MHz, unit="MHz", precision=8),
        ...
        self.setattr_argument("calibration_option", EnumerationValue([
            "Sm1_2_Dm1_2", 
            "Sm1_2_Dm5_2",
            "Radial/vib_freq0_1", 
            "Radial/vib_freq0_2"], 
    ...
    def exp_seq(self, rabi_t, freq_729_dp, ion_status_detect):
        ...
        freq_diff_dp = self.freq_diff_dp*1.0 if ion_status_detect==2 else 0.0
    ...
    def analyze(self):
        ...
        with self.interactive("Ca+ line") as inter:
            inter.setattr_argument(
                "Ca_line", 
                EnumerationValue(["Sm1_2_Dm1_2", 
                                  "Sm1_2_Dm5_2", 
                                  "Radial/vib_freq0_1", 
                                  "Radial/vib_freq0_2"]
                                 , default=self.calibration_option)
            )
        ...
        elif inter.Ca_line == "Radial/vib_freq0_1":
            qubit_freq = self.get_qubit_freq()
            self.parameter_manager.set_param(
                "sideband_Radial/vib_freq0_1",
                -(-qubit_freq+peak*1e6)*2,
                "MHz"
            )

        elif inter.Ca_line == "Radial/vib_freq0_2":
            qubit_freq = self.get_qubit_freq()
            self.parameter_manager.set_param(
                "sideband_Radial/vib_freq0_2",
                -(-qubit_freq+peak*1e6)*2,
                "MHz"
            )
