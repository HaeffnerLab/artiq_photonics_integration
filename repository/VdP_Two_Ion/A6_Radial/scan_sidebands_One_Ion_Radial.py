...
class scan_sidebands_One_Ion_Radial(EnvExperiment):
    ...
    def build(self):
        ...
        self.setattr_argument("enable_fine_line_scan", BooleanValue(True))
        ...
        freq_offset=self.get_dataset("__param__sideband_Radial/vib_freq0_1")/4.0+self.get_dataset("__param__sideband_Radial/vib_freq0_2")/4.0

        self.setattr_argument(
            "freq_offset",
            NumberValue(default=freq_offset, min=-56*MHz, max=250*MHz, unit="MHz", precision=8),
    ...
    def submit_sidebands(self, 
                    line="Radial_BSB_mode_single_ion", #2RSB_mode2
                    # rabi_t=1000,
                    range=0.04, 
                    # awg_power=0.03, 
                    num_points=50,
        ...
        if line == 'Radial_BSB_mode_single_ion':
            freq_scan_center=self.get_dataset("__param__qubit/Sm1_2_Dm5_2")-self.freq_offset
            expid_3_1 = {
                "file": "VdP_Two_Ion/A6_Radial/A6_Radial_rabi_freq_scan.py",
                ...
                "arguments": {
                    ...
                    "att_729_dp": self.get_dataset("__param__sideband_Radial/att_729"),
                    "rabi_t": 20*us,
    ...
    def submit_line(self, 
        ...
        expid_3_1 = {
            "file": "VdP_Two_Ion/A6_rabi_freq_scan_AWG.py",
...
############################################################################################################
        ...
        if self.enable_fine_line_scan:
            rid=self.submit_line('Sm1_2_Dm5_2', rabi_t=1000, range=0.002, awg_power=0.2)
            time.sleep(1)
            while True:
                if self.check_status(rid):
                    break
        ...
        if self.enable_scan_Radial_BSB:
            rid=self.submit_sidebands("Radial_BSB_mode_single_ion")
            time.sleep(1)
            while True:
                if self.check_status(rid):
                    break
        ...
        if self.enable_scan_Radial_RSB:
            rid=self.submit_sidebands("Radial_RSB_mode_single_ion")
            time.sleep(1)
            while True:
                if self.check_status(rid):
                    break
