...
class A7_Cat_Px1x2(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "drive_time",
            NumberValue(default=90.0*us, min=0.001*us, max=300*us,  precision=8, unit='us'),
            tooltip="drive time",
            group='beta grid'
        )
        self.setattr_argument(
            "beta_range_us1",
            NumberValue(default=45.0*us, min=0.001*us, max=300*us,  precision=8, unit='us'),
        ...
        self.setattr_argument(
            "drive_phase_1",
            NumberValue(default=90, min=0, max=360,  precision=8),
            tooltip="drive phase 1",
            group='beta grid'
        )
        self.setattr_argument(
            "drive_phase_2",
            NumberValue(default=90, min=0, max=360,  precision=8),
            tooltip="drive phase 2",
            group='beta grid'
        )
    ...
    def readout_AWG(self, pulse_time1, pulse_time2, freq_729_dp):
        ...
        #SDF 1
        self.seq.sdf_mode1.run(self.drive_time)

        #SDF 2
        self.seq.sdf_mode2.run(self.drive_time)
    ...
    def send_exp_para(self, phase1, phase2):
        ...
        #print(self.freq_mode1_729_sp_aux, self.freq_mode2_729_sp_aux)
        send_exp_para(["Cat2mode_Displacement", {
             
                "freq_readout_RSB1": self.seq.sdf_mode1.freq_729_sp_aux,
                "freq_readout_BSB1": self.seq.sdf_mode1.freq_729_sp,

                "freq_readout_RSB2": self.seq.sdf_mode2.freq_729_sp_aux,
                "freq_readout_BSB2": self.seq.sdf_mode2.freq_729_sp,
                ...
                "phase_readout_mode2": phase2 if not self.Px1p2 else phase2+90,
                "phase1":self.drive_phase_1,

                
                "phase2":self.drive_phase_2 if not self.Px1p2 else self.drive_phase_2+90,
