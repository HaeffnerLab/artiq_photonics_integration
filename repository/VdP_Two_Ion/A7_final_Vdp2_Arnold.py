...
class A7_Sync_VdP2_Arnold(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "phase1",
            NumberValue(default=0.0, min=0.0, max=1.0,  precision=8),
            tooltip="729 single pass amplitude",
            group='phase scan'
        )
        self.setattr_argument(
            "phase2",
            NumberValue(default=0.25, min=0.0, max=1.0,  precision=8),
            tooltip="729 single pass amplitude",
            group='phase scan'
        )
    ...
    def prepare(self):
        ...
        # Create datasets
        self.num_samples = len(self.scan_detuning.sequence)
        num_samples = len(self.scan_detuning.sequence)
        ...
        self.experiment_data.set_list_dataset("phase", num_samples, broadcast=True)
    ...
    @kernel
    def readout_AWG(self, freq_729_dp, detuning_here):
        ...
        self.seq.vdp2mode.run(detuning=detuning_here)
    ...
    @kernel
    def exp_seq(self, freq_729_dp, detuning_here, ion_status_detect):
         #854 repump
        self.seq.repump_854.run()
        #  Cool
        ...
        #apply SDF
        self.readout_AWG(freq_729_dp+freq_diff_dp, detuning_here)

    @rpc(flags={'async'})
    def send_exp_para(self, detuning):
        send_exp_para(["VdP2mode_2D", {
                "freq_VdP_RSB1":self.seq.vdp2mode.Sync_mode1_freq_729_RSB,
                "freq_VdP_RSB2":self.seq.vdp2mode.Sync_mode2_freq_729_RSB+detuning,
                ...
                "freq_readout_RSB2": self.seq.sdf_mode2.freq_729_sp_aux+detuning,
                "freq_readout_BSB2": self.seq.sdf_mode2.freq_729_sp+detuning,
                ...
                "phase_readout_mode1":self.phase1,
                "phase_readout_mode2":self.phase2,
    ...
    def run(self):
        ...
        while i < self.num_samples:
            ...
            detuning_here=self.scan_detuning.sequence[i]
            ...
            if self.seq.adjust_729_freq.check_interval():
                ...
                self.seq.vdp2mode.prepare()
                self.core.break_realtime()
            if self.seq.Check_Motion.check_interval():
                ion_status_detect=self.seq.Check_Motion.calibrate_freq_motion(vib_mode='mode1')
                ion_status_detect=self.seq.Check_Motion.calibrate_freq_motion(vib_mode='mode2')
                ion_status_detect_last=ion_status_detect
                self.seq.vdp2mode.prepare()
            ...
            self.send_exp_para(detuning_here)
            ...
            while sample_num<self.samples_per_time:
                if ion_status_detect==1 or ion_status_detect==2:
                    ...
                    self.exp_seq(freq_729_dp, detuning_here, ion_status_detect)
            ...
            self.experiment_data.append_list_dataset("phase", phase_here)
