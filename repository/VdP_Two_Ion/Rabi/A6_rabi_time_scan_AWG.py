...
class A6_Rabi_Time_AWG_Cam(_ACFExperiment):
    ...
    def prepare(self):
        ...
        self.experiment_data.set_list_dataset(self.prefix+"pmt_counts_avg_thresholded", num_samples, broadcast=True)
        self.experiment_data.set_list_dataset(self.prefix+"rabi_t", num_samples, broadcast=True)
        #self.experiment_data.set_list_dataset('fit_signal', num_freq_samples, broadcast=True)

        # Enable live plotting
        self.experiment_data.enable_experiment_monitor(
            self.prefix+"pmt_counts_avg_thresholded",
            x_data_name=self.prefix+"rabi_t",
            pen=False,
    ...
    def send_exp_para(self):
        if self.enable_pi_pulse:
            send_exp_para(["Two_Pulse",{
                "freq2": self.freq_729_sp,"amp2": self.amp_729_sp, 
                "freq1":self.PI_freq_729_sp, "amp1":self.PI_amp_729_sp,
                "num_loop":max(self.samples_per_time+50,200)}])
        else:
            send_exp_para(["SingleTone",{"freq": self.freq_729_sp,"amp": self.amp_729_sp, "num_loop":max(self.samples_per_time+50,200)}])
    ...
    def run(self):
        ...
        while i < len(self.scan_rabi_t.sequence):
            ...
            self.experiment_data.append_list_dataset(self.prefix+"rabi_t", rabi_t / us)
            self.experiment_data.append_list_dataset(self.prefix+"pos", ion_status_detect)
            self.experiment_data.append_list_dataset(self.prefix+"pmt_counts_avg_thresholded",
    ...
    def analyze(self):
        rabi_time=self.get_dataset(self.prefix+"rabi_t")
        rabi_PMT=self.get_dataset(self.prefix+"pmt_counts_avg_thresholded")
        try:
