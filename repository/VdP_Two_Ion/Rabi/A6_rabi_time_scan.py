...
import time
class A6_Rabi_Time_Cam(_ACFExperiment):
    ...
    def build(self): 
        ...
        # self.power_calibration="__param__VdP2mode/Rabi_Freq_mode1_2RSB"

        self.setattr_argument("power_calibration", 
                              EnumerationValue(["VdP2mode/Rabi_Freq_mode1_2RSB", 
                                                "VdP2mode/Rabi_Freq_mode1_BSB", 
                                                "VdP2mode/Rabi_Freq_mode2_2RSB", 
                                                "VdP2mode/Rabi_Freq_mode2_BSB"
                                                ], default="VdP2mode/Rabi_Freq_mode1_2RSB"))
    ...
    def analyze(self):
        ...
        try:
            ...
            freq_target=self.freq_729_sp
            if self.power_calibration=="VdP2mode/Rabi_Freq_mode1_2RSB":
                freq_target=self.parameter_manager.get_param("frequency/729_sp")+2.0*self.parameter_manager.get_param("VdP2mode/vib_freq1")
            elif self.power_calibration=="VdP2mode/Rabi_Freq_mode1_BSB":
                freq_target=self.parameter_manager.get_param("frequency/729_sp")-self.parameter_manager.get_param("VdP2mode/vib_freq1")
            elif self.power_calibration=="VdP2mode/Rabi_Freq_mode2_2RSB":
                freq_target=self.parameter_manager.get_param("frequency/729_sp")+2.0*self.parameter_manager.get_param("VdP2mode/vib_freq2")
            elif self.power_calibration=="VdP2mode/Rabi_Freq_mode2_BSB":
                freq_target=self.parameter_manager.get_param("frequency/729_sp")-self.parameter_manager.get_param("VdP2mode/vib_freq2")
            
                                                

            fitted_frequency = fitted_omega / (2 * np.pi) * self.scale_sp_power(self.freq_729_sp, freq_target)

            print('frequency after rescale: ', fitted_frequency)
    ...
    def scale_sp_power(self, freq_sp, freq_target):

        freq_data =[]
        volt_data = []


        with open(get_config_dir()/'../repository/Vdp/power_sp.txt', 'r') as f:
            for line in f:
                ...
                volt_data.append(float(y_val))  # Convert y value to float

        # Create the interpolation function
        self.interpolation_function = interp1d(freq_data, volt_data, fill_value="extrapolate", kind='linear')

        return np.sqrt(self.interpolation_function(freq_target)/self.interpolation_function(freq_sp))
