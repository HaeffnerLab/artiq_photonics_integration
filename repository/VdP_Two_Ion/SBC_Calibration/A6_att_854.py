...
class A6_att854_scan_AWG_Cam(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "rabi_t",
            NumberValue(default=60*us, min=0*us, max=5*ms, unit="us",precision=8),
            group="rabi"
        )
        ...
        self.setattr_argument(
            "att_729_dp",
            NumberValue(default=20*dB, min=8*dB, max=31.5*dB, unit="dB", precision=8),
    ...
    def prepare(self):
        ...
        if self.vib_mode=="mode1":
            ...
            self.freq_729_dp+=self.parameter_manager.get_param("VdP1mode/vib_freq")/2.0
        else:
            raise ValueError("Invalid vib mode")
    ...
    @kernel
    def exp_seq(self, att_854_dp, ion_status_detect):
        #854 repump
        ...
        if self.cooling_option == "sidebandcool":
            ...
            if ion_status_detect==2:
                self.seq.sideband_cool_2mode.run(att_854_here=att_854_dp,  freq_diff_dp=self.freq_diff_dp) 
            else:
                self.seq.sideband_cool_2mode.run(att_854_here=att_854_dp)
        
        elif self.cooling_option == "sidebandcool_single_ion":
            self.seq.sideband_cool.run(att_854_here=att_854_dp)
    ...
    def run(self):
        ...
        i=0
        while i < len(self.scan_att_854.sequence):

            total_thresh_count = 0
            sample_num=0
            att_854_dp =self.scan_att_854.sequence[i]
            num_try_save_ion = 0 
            ...
            while sample_num<self.samples_per_time:
                if ion_status_detect==1 or ion_status_detect==2:
                    ...
                    self.exp_seq( att_854_dp, ion_status_detect)
                    ################################################################################
            ...
            self.experiment_data.append_list_dataset("attenuation_dB", att_854_dp/dB)
