...
class A6_Rabi_Time_Tickle_AWG_Cam(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "drive_amp",
            NumberValue(default=0.1, min=0, max=1, precision=8),
            tooltip="amplitude of the drive pulse",
            group='Tickle Trap'
        )
        self.setattr_argument(
            "drive_freq",
            NumberValue(default=self.parameter_manager.get_param("VdP1mode/vib_freq"), min=00*MHz, max=160*MHz, unit="MHz", precision=8),
            tooltip="frequency of the drive pulse",
            group='Tickle Trap'
        )
        ...
        self.setattr_argument(
            "att_729_dp",
            NumberValue(default=self.parameter_manager.get_param("Fock/att_729_dp"), min=8*dB, max=31*dB, unit="dB", precision=8),
            tooltip="729 double pass attenuation",
            group="rabi"
        )
        self.setattr_argument(
            "freq_729_sp",
            NumberValue(default=self.parameter_manager.get_param("frequency/729_sp")-self.parameter_manager.get_param("Fock/freq_BSB_sp"), min=0*MHz, max=160*MHz, unit="MHz", precision=8),
            tooltip="729 double pass frequency",
            group="rabi"
        )
        self.setattr_argument(
            "amp_729_sp",
            NumberValue(default=0.42, min=0, max=1,  precision=8),
            tooltip="729 double pass attenuation",
            group="rabi"
        )
        ...
        self.setattr_argument("cooling_option", EnumerationValue(["sidebandcool", "sidebandcool2mode", "opticalpumping", "sidebandcool_single_ion"], default="sidebandcool_single_ion"))
    ...
    def send_exp_para(self):
        send_exp_para(["Drive_Motion_Single_Pulse",
                       {
                        "num_loop":max(self.samples_per_time+50,200),

                        "freq_sp": self.freq_729_sp,
                        "amp_sp": self.amp_729_sp,
                        "freq_tickle": self.drive_freq,
