...
class A7_Tickle_Sync_VdP2_Wigner(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument("enable_img_readout", BooleanValue(True), group='Pi pulse excitation')
    ...
    def prepare(self):
        ...
        elif self.randomize_option=="sorted":
            self.indices=[i for i in range(len(self.beta_time))]
        else:
            self.indices=[i for i in range(len(self.beta_time))]

    @kernel
    def readout_AWG(self, pulse_time, freq_729_dp, enable_img_readout_here):
        ...
        self.seq.vdp2mode.run(enable_tickle=self.enable_tickle)
    ...
    @kernel
    def exp_seq(self, freq_729_dp, beta_time, enable_img_readout_here, ion_status_detect):
        #854 repump
        self.seq.repump_854.run()
        #  Cool
        ...
        delay(5*us)
        #apply SDF
        self.readout_AWG(beta_time, freq_729_dp+freq_diff_dp, enable_img_readout_here)

    @rpc(flags={'async'})
    def send_exp_para(self, phase_readout, vib_mode, enable_img_readout_here):

        if vib_mode=="mode1":
            send_exp_para(["Drive_Motion_VdP2mode_2D_SDF", {
                    ...
                    #tickle related phase on mode 1
                    "freq_tickle":-(self.seq.sdf_mode1.freq_729_sp/2.0-self.seq.sdf_mode1.freq_729_sp_aux/2.0),
                    "amp_tickle":self.seq.vdp2mode.tickle_amp,
                    "phase_tickle": self.tickle_phase,

                    "enable_tickle":self.enable_tickle,
                   # "num_tickle":0 if not self.enable_tickle else self.seq.vdp2mode.Vdp_Repeat_Time_sync,
                    ...
                    "num_loop":max(self.samples_per_time+200,300)
                }]) 
        elif vib_mode=="mode2":
            send_exp_para(["Drive_Motion_VdP2mode_2D_SDF", {
                    "freq_VdP_RSB1":self.seq.vdp2mode.Sync_mode1_freq_729_RSB,
                    ...
                    "freq_readout_RSB": self.seq.sdf_mode2.freq_729_sp_aux,#+self.freq_detune,
                    "freq_readout_BSB": self.seq.sdf_mode2.freq_729_sp,#-self.freq_detune,
    ...
    def run(self):
        ...
        while i < self.num_samples:
            ...
            for iter_index in range(iter):
                ...
                self.send_exp_para( self.beta_phase[i]*360.0, self.vib_mode, enable_img_readout_here)
