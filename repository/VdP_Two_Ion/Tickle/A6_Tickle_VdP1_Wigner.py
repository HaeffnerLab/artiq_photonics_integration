...
class A6_Tickle_VdP1mode_Wigner_AWG_Cam(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "beta_range_us",
            NumberValue(default=35*us, min=0.001*us, max=300*us,  precision=8, unit='us'),
        ...
        self.setattr_argument("randomize_option", EnumerationValue(["randomize", "sort_by_magnitude","symmetric_randomize","sorted"], default="sort_by_magnitude"))
        self.setattr_argument("enable_tickle", BooleanValue(False))
        self.setattr_argument(
            "tickle_phase",
            NumberValue(default=0, precision=8),
            tooltip="tickle phase",
        )
    ...
    def prepare(self):
        ...
        # Create datasets
        self.experiment_data.set_nd_dataset("pmt_counts", [num_samples*2, self.samples_per_time], broadcast=True)
        ...
        if self.randomize_option=="randomize":
            ...
            self.beta_time, self.beta_phase, self.indices= self.sort_two_arrays_by_magnitude(self.beta_time, self.beta_phase)
        elif self.randomize_option=="symmetric_randomize" and self.readout_option=="2D":
    ...
    def sort_two_arrays_by_magnitude(self, arr1, arr2):
        # Combine the arrays into a list of tuples with their magnitude
        indexed_pairs = [(i, arr1[i], arr2[i], np.sqrt(arr1[i]**2 + arr2[i]**2)) for i in range(len(arr1))]
        ...
        return sorted_arr1, sorted_arr2, original_indices
    ...
    @kernel
    def readout_AWG(self, beta_time, freq_729_dp,  enable_img_readout):
        ...
        self.seq.vdp2mode_one.run(enable_tickle=self.enable_tickle)
    ...
    def exp_seq(self, freq_729_dp, beta_time, enable_img_readout, ion_status_detect):
        ...
        # rabi 
        self.readout_AWG(beta_time, freq_729_dp+freq_diff_dp, enable_img_readout)
    ...
    def send_exp_para(
        self,
        phase,
        enable_img_readout_here
    ):
        if self.vib_mode=="mode_single_ion":
            send_exp_para(["Drive_Motion_VdP_SDF", {
                        #SDF related phase
                        ...
                        #tickle related phase
                        "freq_tickle":-(self.seq.sdf_single_ion.freq_729_sp/2.0-self.seq.sdf_single_ion.freq_729_sp_aux/2.0),
                        "amp_tickle":self.seq.vdp2mode_one.tickle_amp,
                        "phase_tickle": self.tickle_phase,
                        "num_tickle":0 if not self.enable_tickle else self.seq.vdp2mode_one.tickle_num, 

                        #rotate in the sigma_y basis
                        "freq_sp_pi":self.PI_freq_729_sp,
                        "amp_sp_pi":self.PI_amp_729_sp,
                        "phase_sp_pi":90.0,
                        "enable_pi_pulse":enable_img_readout_here,

                        "num_loop":max(self.samples_per_time+100,300)
                    }]) 
        elif self.vib_mode=="mode1":
            send_exp_para(["Drive_Motion_VdP_SDF", {
                        #SDF related phase
        ...
        while i < self.num_samples:
            ...
            for iter_index in range(iter):
                ...
                if self.seq.calibrate_motion.check_interval():
                    ion_status_detect=self.seq.calibrate_motion.calibrate_freq_motion(vib_mode=self.vib_mode, use_motion_tracking=True)
                    ion_status_detect_last=ion_status_detect
                ...
                self.seq.sdf_single_ion.prepare(use_motion_tracking=True)
                self.seq.sdf_mode1.prepare(use_motion_tracking=True)
                self.seq.sdf_mode2.prepare(use_motion_tracking=True)
                self.seq.vdp2mode_one.prepare(self.vib_mode)
                ...
                while sample_num<self.samples_per_time:
                    if ion_status_detect==1 or ion_status_detect==2:
                        ...
                        if (ion_status_detect==ion_status_detect_last and ion_status_detect==1): #ion shouldn't move
                            ...
                            self.mutate_dataset("pmt_counts",
                                                ((index+iter_index*self.num_samples,index+iter_index*self.num_samples+1),(sample_num, sample_num+1)),
                                                (1 if ion_status==0 else 0))
