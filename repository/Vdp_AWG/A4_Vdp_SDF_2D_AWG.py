...
class A4_Vdp_Wigner_2D_AWG(_ACFExperiment):
    ...
    def prepare(self):
        ...
        self.experiment_data.set_nd_dataset("pmt_counts", [self.num_samples, self.samples_per_time])
    ...
    @kernel
    def rabi_AWG(self, pulse_time,  att_729_dp):
        ...
        while i < self.num_samples:
            ...
            while sample_num<self.samples_per_time:
                if is_ion_good:
                    ...
                    self.seq.sideband_cool.run(freq_offset=0.526*MHz)
                    self.seq.sideband_cool.run(freq_offset=0.914*MHz, att_729_here=18.0*dB)
                    delay(5*us)
                    ...
                    #apply SDF
                    self.rabi_AWG(self.beta_time[i],self.att_729_dp)
