...
class Raman_SBC_VibFreqScan(_ACFExperiment):
    ...
    def build(self):
        ...
        self.setattr_argument(
            "RSB_drive_time",
            NumberValue(default=2000*us, min=0.*us, max=5000*us, unit='us'),
        ...
        self.setattr_argument(
            ...
            Scannable(
                default=RangeScan(
                    start=self.parameter_manager.get_param("sideband_Raman/vib_freq1")-0.05*MHz,
                    stop=self.parameter_manager.get_param("sideband_Raman/vib_freq1")+0.05*MHz,
                    npoints=50
                ),
                 unit='MHz',
                global_min=0.*MHz,
                global_max=100.0*MHz,
                global_step=0.01*MHz
            ),
    ...
    def prepare(self):
        self.fitting_func.setup(len(self.scan_vib_freq.sequence))
        # Create datasets
        num_freq_samples = len(self.scan_vib_freq.sequence)
    ...
    def run(self):
        ...
        while time_i < len(self.scan_vib_freq.sequence):
            
            freq_vib=self.scan_vib_freq.sequence[time_i]
            ...
            while sample_num<self.samples_per_time:
                if is_ion_good:
                    ...
                    self.seq.sideband_Raman.run(freq_vib1=freq_vib)
            ...
            self.experiment_data.append_list_dataset("frequency", freq_vib)
