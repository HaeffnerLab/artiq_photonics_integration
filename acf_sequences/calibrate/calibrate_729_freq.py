...
class Check_729_Freq(Sequence):
    ...
    def build(self):
        ...
        self.setattr_argument(
            "cal_num_samples_per_freq",
            NumberValue(default=35, precision=0, step=1),
            tooltip="Number of samples to take for each time",
            group="calibrate_qubit"
        )
        self.setattr_argument(
            "cal_freq_range",
            NumberValue(default=0.0015*MHz, min=0*MHz, max=160*MHz, unit="MHz", precision=8),
            tooltip="729 double pass frequency",
            group="calibrate_qubit"
        )
        self.setattr_argument(
            "cal_rabi_t",
            NumberValue(default=1500*us, min=0*us, max=5*ms, unit="us",precision=8),
            group="calibrate_qubit",
        ...
        self.setattr_argument(
            "cal_amp_729_sp",
            NumberValue(default=0.01, min=0, max=1,  precision=8),
            tooltip="729 double pass attenuation",
            group="calibrate_qubit"
        )
        self.setattr_argument(
            "cal_interval",
            NumberValue(default=400*s, min=0*s, max=500*s, unit="s",precision=8),
            group="calibrate_qubit",
    ...
    def send_cal_para(self):
        send_exp_para(["SingleTone",{"freq": self.cal_freq_729_sp,"amp": self.cal_amp_729_sp, "num_loop":max(self.cal_num_samples_per_freq+100,100)}])

    def get_qubit_freq(self)->float:
        #if self.line=="Sm1_2_Dm5_2":
        return self.exp.get_dataset('__param__qubit/Sm1_2_Dm5_2', archive=False)
        # else:
        #     return self.exp.get_dataset('__param__qubit/Sm1_2_Dm1_2', archive=False)
    ...
    def calibrate_freq_qubit(self, 
        ...
        while i < self.cal_num_samples:
            ...
            if self.seq.awg_trigger.run(self.core, self.core.seconds_to_mu(50*ms), self.core.seconds_to_mu(50*us) ) <0 : 
                print("BAD AWG check 729 freq!!!!")
                continue
           

            while sample_num<self.cal_num_samples_per_freq:
            ...
            self.experiment_data.insert_nd_dataset("cal_count", i, 
                                        float(total_thresh_count) / self.cal_num_samples_per_freq)

            self.experiment_data.insert_nd_dataset("cal_pos",i, ion_status_detect)
    ...
    @rpc
    def update_729_freq(self):
        freq=self.exp.get_dataset("cal_freq")
        PMT_count=self.exp.get_dataset('cal_count')
        ...
        try:
            ...
            if is_success and np.abs(peak*MHz-freq_729_dp)<self.cal_freq_range:
                self.exp.set_dataset('cal_fit_signal', fitted_array, broadcast=True)
