...
class A4_Vdp_Wigner_2D_AWG(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "Vdp_freq_729_dp",
            NumberValue(default=self.parameter_manager.get_param("qubit/Sm1_2_Dm5_2"), min=180*MHz, max=250*MHz, unit="MHz", precision=8),
        ...
        self.setattr_argument(
            "Vdp_freq_729_BSB_sp",
            NumberValue(default=self.parameter_manager.get_param("frequency/729_sp")-self.parameter_manager.get_param("qubit/vib_freq"), min=10*MHz, max=250*MHz, unit="MHz", precision=8),
            tooltip="729 double pass frequency for resonance",
            group='VdP Hamiltonian'
        )
        self.setattr_argument(
            "Vdp_freq_729_2RSB_sp",
            NumberValue(default=self.parameter_manager.get_param("frequency/729_sp")+self.parameter_manager.get_param("qubit/vib_freq"), min=10*MHz, max=250*MHz, unit="MHz", precision=8),
            tooltip="729 double pass frequency for resonance",
            group='VdP Hamiltonian'
        )
        self.setattr_argument(
            ...
            NumberValue(default=13*dB, min=5*dB, max=30*dB, unit="dB", precision=6),
            tooltip="729 single pass amplitude",
            group='VdP Hamiltonian'
        )
        self.setattr_argument(
            "Vdp_drive_att_BSB_sp",
            NumberValue(default=20*dB, min=5*dB, max=30*dB, unit="dB", precision=6),
            tooltip="729 single pass amplitude",
            group='VdP Hamiltonian'
        )
    ...
    def Vdp_Evolve(self):
        ...
        for i in range(self.Repeat_Time):
            ...
            self.dds_729_sp.set(self.Vdp_freq_729_2RSB_sp, phase=0.)
            ...
            self.dds_729_sp.set(self.Vdp_freq_729_BSB_sp, phase=0.)
