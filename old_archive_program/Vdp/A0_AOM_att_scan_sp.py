...
class A0_AOM_att_scan_sp(_ACFExperiment):
    def build(self):
    ...
    def prepare(self):
        ...
        # Create datasets
        num_samples = len(self.scan_att_729_sp.sequence)
    ...
    def run(self):
        ...
        for att in self.scan_att_729_sp.sequence:

            self.dds_729_sp.set_att(att)
            self.dds_729_dp.sw.on()
            delay(1*ms)
            ...
            self.experiment_data.append_list_dataset("attenuation_dB", att/dB)
