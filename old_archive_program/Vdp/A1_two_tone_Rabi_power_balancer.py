...
class A1_two_tone_Att_scan_sp_aux(_ACFExperiment):
    ...
    def build(self): 
        ...
        self.setattr_argument(
            "freq_729_dp1",
            NumberValue(default=self.parameter_manager.get_param("qubit/Sm1_2_Dm5_2")+self.parameter_manager.get_param("qubit/vib_freq")
        ...
        self.setattr_argument(
            "att_729_sp",
            NumberValue(default=self.parameter_manager.get_param("attenuation/729_sp")+0.5*dB, min=10*dB, max=30*dB, unit="dB", precision=8),
        ...
        self.setattr_argument(
            ...
            Scannable(
                default=RangeScan(
                    start=13.5*dB,#self.parameter_manager.get_param("attenuation/729_sp")-1*dB,
                    stop=15*dB, #self.parameter_manager.get_param("attenuation/729_sp")+4*dB,
                    npoints=20
                ),
                global_min=8*dB,
                global_max=31.5*dB,
    ...
    def prepare(self):
        self.fitting_func.setup(len(self.scan_att_729_sp_aux.sequence))
        # Create datasets
        num_freq_samples = len(self.scan_att_729_sp_aux.sequence)
    ...
    def run(self):
        ...
        delay(200*us)
        for att_i in range(len(self.scan_att_729_sp_aux.sequence)): 

            total_thresh_count1 = 0
            total_thresh_count2 = 0

            sample_num=0
            att_sp_aux = self.scan_att_729_sp_aux.sequence[att_i]
            ...
            while sample_num<self.samples_per_time:
                ...
                self.dds_729_dp.set(self.freq_729_dp2)
                self.dds_729_dp.set_att(self.att_729_dp2)
            ...
            self.experiment_data.append_list_dataset("att_sp_aux", att_sp_aux / dB)
    ...
    def analyze(self):
        ...
        print("find power balance at att_sp_aux = ", x_zero, " dB")
